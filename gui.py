...
import time
title="Ship classification From SAR Images"
path1="sample.jpg"
path2="sample1.jpg"
main_color='#271745'  
...
from model_class import ShipModel
from calibrate import lbp2
import numpy as np

def process1(path2,lb1):
    if path1=='sample.jpg':
        messagebox.showinfo('error','please choose an image')
        return
    global f6,f2,top
    ...
    ret=model.predict_type(img)
    print(ret)
    lb1.after(10,delayed_insert,lb1,0,'Loading image')
    lb1.after(10,delayed_insert,lb1,1,'Loading model')
    lb1.after(10,delayed_insert,lb1,2,'Calibrate image')
    lb1.after(10,delayed_insert,lb1,3,'Prediction')
    ...
    l1=Label(f6,text="Result",font="Helvetica 13 bold")
    l1.pack(side="bottom",fill="both")
    lb1.after(1000,delayed_display)
    lb2.after(2000,showresult,ret)
...
if __name__=="__main__":
    ...
    f.config(bg=main_color)
    f.pack(side="top", fill="both", expand=True,padx=10,pady=10)
    l=Label(f,text="Ship classification from SAR images ",font = "Verdana 30 bold",fg="white",bg=main_color)
    l.place(x=50,y=50)
